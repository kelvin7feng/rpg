...
namespace NodeCanvas.Editor
{
    ...
    public static class BBParameterEditor
    {
        ...
        ///An editor for BBParameter type to let user choose either a constant value or link to a Blackboard Variable.
        public static BBParameter ParameterField(GUIContent content, BBParameter bbParam, bool blackboardOnly = false, object[] attributes = null, object context = null) {
            ...
            if ( !blackboardOnly && !bbParam.useBlackboard ) {
                ...
                GUILayout.BeginVertical();
                if (EditorCustomUtils.IsLuaClass(content))
                {
                    EditorCustomUtils.GenericPopup(content, bbParam, context);
                }
                else {
